...
export default class ActivityPage {
  ...
  generateMonthButtons() {
    ...
    monthsToGenerate.forEach((label) => {
      ...
      btn.addEventListener("click", () => {
        ...
        const [month, year] = label.split(" ");
        this.showBooksForMonth(month, year);
        this.updateStatistics(parseInt(year));
      });
  ...
  updateStatistics(year) {
    ...
    this.allBooks.forEach((book) => {
      const yearLogs = (book.logs || []).filter((log) => {
        const logDate = new Date(log.date);
      ...
      yearLogs.sort((a, b) => new Date(a.date) - new Date(b.date));
      const lastPage = yearLogs[yearLogs.length - 1].page;

      totalPages += lastPage;
      if (lastPage >= book.totalPages) booksFinished++;
    });
    ...
    statsContainer.innerHTML = `
      ...
      <div class="streak-item">
        ...
        <p class="stat-value">${booksFinished}</p>
      </div>
      <div class="streak-item">
        <p>Pages read in ${year}</p>
